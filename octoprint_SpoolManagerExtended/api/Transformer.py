...
def calculateRemainingWeight(usedWeight, totalWeight):
    ...
    if isinstance(usedWeight, (int, float)) and isinstance(totalWeight, (int, float)):
...
def _calculateRemainingPercentage(remainingWeight, totalWeight):
    ...
    if (
        isinstance(remainingWeight, (int, float))
        and isinstance(totalWeight, (int, float))
        and (totalWeight > 0)
    ):
...
def _calculateUsedPercentage(usedWeight, totalWeight):
    ...
    if (
        isinstance(usedWeight, (int, float))
        and isinstance(totalWeight, (int, float))
        and (totalWeight > 0)
    ):
