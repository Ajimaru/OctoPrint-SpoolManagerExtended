...
__copyright__ = (
    "Copyright (C) 2017 Sven Lohrmann - Released under terms of the AGPLv3 License"
)
...
class FilamentOdometer(object):
    ...
    def reset(self):
        ...
        self.allToolExtrusions = {
            self.currentToolName: self.ToolExtrusion()
        }  # Tool0 must always be there, default extruder
    ...
    def parse(self, gcode, cmd):
        ...
        if gcode == "G1" or gcode == "G0":  # move
            ...
            if e is not None:
                ...
                self._getCurrentToolExtrusion().maxExtrusion = max(
                    self._getCurrentToolExtrusion().maxExtrusion,
