...
#
# Run with:  python3 octoprint_SpoolManagerExtended/test/test_U1RfidManager.py
...
# U1RfidManager.py does `from octoprint_SpoolManagerExtended.common.SettingsKeys import
# SettingsKeys` - an absolute import of the real package, which would otherwise drag in
# octoprint_SpoolManagerExtended/__init__.py (flask, OctoPrint) just to reach a dependency-free
...
def _installNamespacePackages():
    if "octoprint_SpoolManagerExtended" not in sys.modules:
        pluginPackage = types.ModuleType("octoprint_SpoolManagerExtended")
        pluginPackage.__path__ = [_PLUGIN_ROOT]
        sys.modules["octoprint_SpoolManagerExtended"] = pluginPackage
    if "octoprint_SpoolManagerExtended.common" not in sys.modules:
        commonPackage = types.ModuleType("octoprint_SpoolManagerExtended.common")
        commonPackage.__path__ = [os.path.join(_PLUGIN_ROOT, "common")]
        sys.modules["octoprint_SpoolManagerExtended.common"] = commonPackage
...
class TestDetectionChain(unittest.TestCase):
    ...
    def test_nonMoonrakerPrinterStopsAtStageTwo(self):
        # e.g. an instance running bambu_connector instead
        manager, plugin = _makeManager(
