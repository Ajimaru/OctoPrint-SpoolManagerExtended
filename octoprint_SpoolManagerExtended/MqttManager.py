...
class MqttManager(object):
    ...
    def __init__(self, plugin, logger):
        ...
        self._lastDiscoveryIdentity = (
            None  # (prefix, base, instance, toolCount) of last published discovery
        )
    ...
    def initialize(self):
        ...
        if helpers and "mqtt_publish" in helpers:
            ...
            self._mqtt_publish = None
            self._logger.info(
                "MQTT plugin not available, SpoolManager MQTT support disabled"
            )
    ...
    def _isDiscoveryEnabled(self):
        return self._plugin._settings.get_boolean(
            [SettingsKeys.SETTINGS_KEY_MQTT_DISCOVERY_ENABLED]
        )

    def _getRetain(self):
        return self._plugin._settings.get_boolean(
            [SettingsKeys.SETTINGS_KEY_MQTT_RETAIN]
        )

    def _getDiscoveryPrefix(self):
        prefix = self._plugin._settings.get(
            [SettingsKeys.SETTINGS_KEY_MQTT_DISCOVERY_PREFIX]
        )
    ...
    def _getTopicBase(self):
        ...
        return (base or "octoprint/plugin/SpoolManagerExtended").strip().strip("/")

    @staticmethod
    def _slugify(value):
        return (
            re.sub(r"[^a-z0-9_]+", "_", value.strip().lower()).strip("_") or "octoprint"
        )

    def _getInstanceName(self):
        instanceName = self._plugin._settings.get(
            [SettingsKeys.SETTINGS_KEY_MQTT_INSTANCE_NAME]
        )
    ...
    def _discoveryTopic(self, discoveryPrefix, instanceName, toolIndex, objectId):
        return "%s/sensor/spoolmanager_%s/tool%d_%s/config" % (
            discoveryPrefix,
            instanceName,
            toolIndex,
            objectId,
    ...
    def _buildDiscoveryPayload(self, topicBase, instanceName, toolIndex, objectId):
        ...
        configPayload = {
            ...
            "unique_id": "spoolmanager_%s_tool%d_%s"
            % (instanceName, toolIndex, objectId),
    ...
    def publishDiscovery(self):
        ...
        for toolIndex in range(toolCount):
            for objectId in self.SENSOR_DEFINITIONS:
                topic = self._discoveryTopic(
                    discoveryPrefix, instanceName, toolIndex, objectId
                )
                ...
                self._publish(
                    topic,
                    self._buildDiscoveryPayload(
                        topicBase, instanceName, toolIndex, objectId
                    ),
                    force=True,
                    retained=True,
                )
        ...
        self._logger.info(
            "Published MQTT discovery configs for %d tool(s) as instance '%s'"
            % (toolCount, instanceName)
        )
    ...
    def publishToolState(self, toolIndex, eventPayload):
        ...
        self._publish(
            topic, self._buildStatePayload(instanceName, toolIndex, eventPayload)
        )
    ...
    def clearRetainedTopics(
        self, discoveryPrefix=None, topicBase=None, instanceName=None, toolCount=None
    ):
        if self._mqtt_publish is None:
            return
        if self._lastDiscoveryIdentity is not None:
            lastPrefix, lastBase, lastInstance, lastToolCount = (
                self._lastDiscoveryIdentity
            )
        ...
        for toolIndex in range(toolCount):
            ...
            self._publish(
                self._stateTopic(topicBase, instanceName, toolIndex),
                "",
                force=True,
                retained=True,
            )
        ...
        self._logger.info(
            "Cleared retained MQTT topics for instance '%s'" % instanceName
        )
