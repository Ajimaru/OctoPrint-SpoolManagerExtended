...
from octoprint_SpoolManager.DatabaseManager import DatabaseManager, MODELS
...
class TestDumpImportValidation(unittest.TestCase):
	...
	def _parse(self, statement):
		return self.databaseManager._parseInsertDumpStatement(statement, self.allowedModels)
	...
	def test_splitRealisticDump(self):
		dumpText = "\n".join([
			...
			") ENGINE=InnoDB;",
			"",
			"INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `displayName`) VALUES (1, 'PLA');",
		])
		...
		self.assertTrue(statements[2].startswith("CREATE TABLE `" + SPOOL_TABLE + "` ("))
		self.assertTrue(statements[3].startswith("INSERT INTO"))

	def test_ignoredStatements(self):
		self.assertTrue(self.databaseManager._isIgnoredDumpStatement("SET NAMES utf8mb4;", self.allowedModels))
		self.assertTrue(self.databaseManager._isIgnoredDumpStatement(
		...
		self.assertFalse(self.databaseManager._isIgnoredDumpStatement(
	...
	def test_parseSimpleInsert(self):
		parsed = self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `displayName`, `totalWeight`) "
	...
	def test_parseNullAndNumbers(self):
		parsed = self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `vendor`, `offsetTemperature`, `density`) "
							 "VALUES (7, NULL, -15, 1e-05);")
		self.assertIsNotNone(parsed)
		self.assertEqual([7, None, -15, 1e-05], parsed[2])

	def test_parseEscapedQuotes(self):
		parsed = self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `displayName`, `vendor`) "
							 "VALUES (1, 'O\\'Reilly', 'It''s');")
		self.assertIsNotNone(parsed)
		self.assertEqual([1, "O'Reilly", "It's"], parsed[2])

	def test_parseBackslashSequences(self):
		parsed = self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `noteText`) "
							 "VALUES (1, 'line1\\nline2\\\\end\\Z');")
		self.assertIsNotNone(parsed)
		self.assertEqual("line1\nline2\\end\x1a", parsed[2][1])

	def test_parseDatetimeAndEmoji(self):
		parsed = self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `created`, `colorName`) "
	...
	def test_injectionPayloadInsideLiteralStaysData(self):
		parsed = self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `displayName`) "
							 "VALUES (1, '\\'); DROP TABLE " + SPOOL_TABLE + ";--');")
		self.assertIsNotNone(parsed)
		self.assertEqual("'); DROP TABLE " + SPOOL_TABLE + ";--", parsed[2][1])

	def test_parseMetaTableInsert(self):
		parsed = self._parse("INSERT INTO `" + META_TABLE + "` (`databaseId`, `key`, `value`) "
	...
	def test_rejectSubquery(self):
		self.assertIsNone(self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `displayName`) "
									  "VALUES (1, (SELECT password FROM users));"))

	def test_rejectFunctionCall(self):
		self.assertIsNone(self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `created`) "
									  "VALUES (1, NOW());"))
	...
	def test_rejectUnknownTable(self):
		self.assertIsNone(self._parse("INSERT INTO `mysql_user` (`databaseId`, `displayName`) VALUES (1, 'PLA');"))

	def test_rejectUnknownColumn(self):
		self.assertIsNone(self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `evilColumn`) VALUES (1, 'x');"))

	def test_rejectDuplicateColumn(self):
		self.assertIsNone(self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `displayName`, `displayName`) "
									  "VALUES (1, 'a', 'b');"))

	def test_rejectColumnValueCountMismatch(self):
		self.assertIsNone(self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`, `displayName`) VALUES (1);"))
		self.assertIsNone(self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`databaseId`) VALUES (1, 'x');"))

	def test_rejectMissingLeadingDatabaseId(self):
		self.assertIsNone(self._parse("INSERT INTO `" + SPOOL_TABLE + "` (`displayName`, `databaseId`) "
									  "VALUES ('PLA', 1);"))
	...
	def test_safeIdentifierAccepted(self):
		self.assertEqual(SPOOL_TABLE, self.databaseManager._assertSafeSQLIdentifier(SPOOL_TABLE))
		self.assertEqual("databaseId", self.databaseManager._assertSafeSQLIdentifier("databaseId"))

	def test_unsafeIdentifierRejected(self):
		for badIdentifier in ["spo`ol", "a b", "a;b", "a-b", "a.b", "", "col`); DROP TABLE x;--"]:
	...
	def test_unescapeRoundtrip(self):
		self.assertEqual("It's a \"test\"\n\t\\",
						 self.databaseManager._unescapeMySQLStringLiteral("It\\'s a \\\"test\\\"\\n\\t\\\\"))
		self.assertEqual("It's", self.databaseManager._unescapeMySQLStringLiteral("It''s"))
		self.assertEqual("\0\x1a", self.databaseManager._unescapeMySQLStringLiteral("\\0\\Z"))
